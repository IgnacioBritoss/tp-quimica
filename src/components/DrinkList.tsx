...
export function DrinkList({ entries, onRemove, onQuantityChange }: DrinkListProps) {
  if (entries.length === 0) {
        ...
        {entries.map((entry) => (
          ...
          >
            <div
              ...
              style={{ backgroundColor: `${entry.color.startsWith("var") ? "#1c5aa6" : entry.color}1f` }}
            >
              <DrinkImage
                src={entry.image}
                icon={entry.icon}
                color={entry.color.startsWith("var") ? "#1c5aa6" : entry.color}
                alt={entry.name}
